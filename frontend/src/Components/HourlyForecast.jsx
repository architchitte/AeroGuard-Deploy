  ...
  return (
    <div className="bg-gradient-to-br from-slate-800 to-slate-700 rounded-2xl p-6 shadow-2xl border border-slate-700">
      <div className="flex items-center gap-2 mb-6">
        <h3 className="text-lg font-bold text-white">6-Hour Forecast</h3>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
        {forecasts.map((item, idx) => (
          ...
          >
            <p className="text-slate-300 text-xs font-semibold mb-2">{item.hour}</p>
            <div className={`${getColor(item.aqi)} w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-2 shadow-lg`}>
              <span className="text-white font-bold text-sm">{item.aqi}</span>
            </div>
            <p className="text-slate-400 text-xs">{item.trend === 'up' ? '📈' : '📉'}</p>
          </div>
        ))}
      </div>
    </div>
